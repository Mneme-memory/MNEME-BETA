...
**Version**: 15.1.0 | Updated July 2026 | For people building memory systems
...
- [Startup and release maintenance](#startup-and-release-maintenance)
...
10. Background queue (does not block the response):
    ├── generate_embedding(user_message)     ← local BGE ONNX by default (OpenAI if embedding_provider: openai)
...
|------|----------|---------------|
| **Active** | Recent messages within the `context.recent_messages_tokens` budget (default 50k) | Loaded verbatim into conversation history — always in context |
| **Standard** | Messages that have aged out of the active window | Semantic search only |
| **Deep archive** | Standard messages ≥6 months old (age-based only, no importance filter) | Semantic search with `@recall --deep` flag only |
...
The infographic shows five context layers in parallel cards. In practice, assembly is sequential, and the layers share one elastic token pool rather than each getting a fixed slice.

```
assemble_context(recent_messages, user_query)
    │
    ├─ 1. RECENT HISTORY         context.recent_messages_tokens (default 50,000)
    │       Always included, verbatim. This is the conversation window.
    │       Everything below draws from what's left of the model's context
    │       window after this, the current input, and fixed reserves for
    │       AI Notes / Timeline / Attachments (see prompt budget below).
    │
    ├─ 2. SEMANTIC MEMORY        elastic — shares the dynamic pool with entity summaries
    ...
    │
    ├─ 3. ENTITY SUMMARIES       min(features.entity_summaries.max_tokens [5,000], 1/4 of the dynamic pool)
    │       Derived from step 2 — runs after retrieval, not independently.
    │       Takes its slice off the dynamic pool first; memory gets the rest.
    │       (See: Entity summaries section below)
    │
    ├─ 4. NARRATIVE CONCEPTS     whatever remains of the dynamic pool after 2+3
    │       Keyword scan of last N messages (default: 5).
    │       Independent of retrieval results; typically renders to ~500-1000 tokens.
    │
    ├─ 5. FILE ATTACHMENTS       whatever remains of the dynamic pool after 2+3+4
    │       Recently attached/described files, budget-trimmed.
    │
    ├─ 6. AI NOTES               fixed reserve: features.notes.max_tokens (default 5,000)
    │       All notes loaded verbatim from database.
    │       No filtering or ranking — all-or-nothing.
    │
    └─ 7. TIMELINE               fixed reserve: days_to_show × max_tokens_per_day + 500 (default 7×400+500 = 3,300)
            7-day daily summary block, loaded verbatim. Cached (see below).
...
Unlike memory/entities/concepts/attachments (which share one elastic pool computed per-turn from remaining context-window space), AI Notes and Timeline are reserved as fixed slices off the top before the elastic pool is sized — they don't compete with the other layers for space. Source: `_split_dynamic_budget` / `_dynamic_budget_remaining` in `context.py`, and the budget calculation in `conversation.py` (`_current_prompt_budget` reservations).
...
This is the base formula when no graph DB is available. In practice `GraphDatabase` is initialized unconditionally on every install (`_build_system()` in `server.py`) and `retrieval.entity_match_weight` defaults to 0.15 in `config.example.json`, so the four weights are active by default and rescaled to sum to 1:

```
Final_Score = (semantic × ~0.34) + (importance × ~0.34) + (recency × ~0.17) + (entity_match × 0.15)
```

`entity_match` combines keyword hits in the message content with entity-link confidence when an entity name/alias from the query is recognized. Setting `entity_match_weight` to 0 disables the fourth term and the other three fall back to the 0.4/0.4/0.2 split shown above.

Each component:

**Semantic similarity** — cosine similarity between query embedding and message embedding. Default provider is local BGE (384 dimensions, threshold ~0.50); OpenAI `text-embedding-3-small` (1536 dimensions, threshold 0.25) is the legacy fallback for `embedding_provider: openai`. Threshold is configurable. Messages below threshold are dropped entirely.
...
Either the user or the AI can mark a message via `@remember {id}`. Explicitly marked messages are set straight to the maximum importance score:

```
new_importance = 10.0
```

Earlier versions used subtle multipliers, but that was too easy for important memories to lose priority after later tier transitions. The current behavior treats an explicit remember as a clear pin for retrieval ranking.
...
Each retrieved memory has entity links (assigned by Haiku during background processing). All linked entities are collected and their scores are accumulated: `avg_confidence × log(link_count + 1) × recency_factor × memory_count × type_priority`, where `type_priority` is 2.0 for CapitalCase (concrete: people/projects/events) names and 1.0 for lowercase (abstract theme) names.
...
Response finalized
    │
    ├── generate_embedding(user_message)      ← local BGE ONNX by default (OpenAI if embedding_provider: openai)
    ├── generate_embedding(assistant_message) ← local BGE ONNX by default (OpenAI if embedding_provider: openai)
...
InstanceExecutor extracts @run block in pre-pass (same as @note/@artifact)
    │
    ▼
cmd_run() writes code to a file, launches the bundled native runner in a
Windows AppContainer sandbox (sandbox_runner.py) with env vars:
    - DB_PATH (read-only SQLite URI, ?mode=ro)
    - REPO_PATH, SRC_PATH, CONFIG_PATH (redacted), ARTIFACTS_PATH,
      ATTACHMENTS_PATH, SCRATCH_PATH
    │
    ▼
stdout/stderr captured → returned as retrieval result → continuation
    │
    ▼
@run...@endrun block stripped from stored response (replaced with [looked it up])
```

Database access is read-only at the SQLite engine level via the `?mode=ro` URI, on top of the sandbox's own read-only filesystem grants. 10-second timeout, 4000-char output truncation.
...
## Startup and release maintenance

Server startup now exposes a lightweight readiness flow for the tray app and browser loading page:

```
tray.py starts server
    │
    ├── /api/startup/status reports starting / ready / failed
    ├── startup.html polls status and shows progress
    └── normal app routes wait until initialization finishes
```

After core initialization, maintenance work runs without blocking chat availability:
- optional SQLite snapshot backup via `Database.create_backup()` when `system.enable_auto_backup` is true
- pruning old snapshots according to `system.backup_keep_count`
- optional copy to `storage.cloud_backup_path`
- startup entity backfill for missed messages

Production release publishing intentionally uses orphan snapshots. `scripts/push-production.sh` force-pushes the current `MNEME-MAIN` tree to `Mneme-memory/MNEME-BETA` without personal history and rebuilds `Mneme-release.zip`. Because there is no shared history, compare production drift with `scripts/check-production-sync.sh`, which diffs the two trees directly.
...
| Code execution (@run) | `src/backend/commands.py` (`cmd_run`), `src/backend/sandbox_runner.py` |
...
| Startup status / backups | `src/backend/server.py`, `src/backend/database.py` |
| Release snapshot tooling | `scripts/push-production.sh`, `scripts/check-production-sync.sh` |
| Database schema | `src/backend/schema.py` |

Cross-cutting flow notes for maintainers live in the development repo's `.claude/skills/mneme/` tree. They are omitted from public release snapshots.

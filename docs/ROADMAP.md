...
### Native AI Tool Use
- **Replace the `@command` protocol with native tool use** — give the model typed, structured tools for memory retrieval, concepts, files, artifacts, notes, and code execution instead of parsing commands out of assistant prose and making fragile continuation round trips. This is the top architectural priority: clearer schemas and permission boundaries, more reliable execution, and one tool layer that user-facing shortcuts can also call into.

...
Off by default — set `system.debug_endpoints: true` in `config.json` to enable (returns 403 otherwise).
...
For system instructions specifically, the easy path is now Settings → Profile settings → System instructions in the app; hand-editing `system_instructions.txt` directly is still there as an advanced fallback.

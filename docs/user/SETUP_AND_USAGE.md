...
**Windows:** Double-click `install-windows.bat` in the Mneme folder.

**macOS / Linux:** Open a terminal in the Mneme folder and run `bash install-mac-linux.sh`.

The installer handles Python dependency installation, config file creation, and shortcut setup. On Windows, it also offers to install Tailscale for mobile access.
...
**macOS / Linux:** Run `python3 scripts/tray.py` or launch from application menu.
...
Your data is in `data/{profile}/`. Back up this folder to preserve conversations, artifacts, and attachments.

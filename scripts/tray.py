...
FIRST_LAUNCH_MARKER = PROJECT_ROOT / "data" / ".launched"
...
def sync_icon_state():
    ...
    if running == _last_known_state:
        return  # No change

    _last_known_state = running
    if running:
...
def start_server(icon=None, item=None):
    ...
    if server_running() and tray_icon:
        tray_icon.notify("Mneme server started", "Mneme")
    elif tray_icon:
...
def server_status_text(item):
    return "Server: running" if server_running() else "Server: stopped"

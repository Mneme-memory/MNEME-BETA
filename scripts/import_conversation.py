...
def generate_embeddings(db: Database, inserted: list, config: dict):
    """Generate embeddings (local or OpenAI, per retrieval.embedding_provider) for all inserted messages."""
    print(f"\n{'='*60}")
    print("STEP 2: Generating embeddings")
    print(f"{'='*60}")

    embedding_provider = config.get("retrieval", {}).get("embedding_provider", "openai")
    if embedding_provider != "local" and not config["api_keys"].get("openai"):
        print("  Skipping: No OpenAI API key configured")
        return

    embedder = create_embedding_generator(config)
    success = 0
    failed = 0
...
def estimate_costs(message_count: int, db: Database = None, graph_db: GraphDatabase = None, embedding_provider: str = "openai"):
    """Print cost estimates for the enrichment pipeline."""
    batches = (message_count + 29) // 30

    embed_cost = 0.0 if embedding_provider == "local" else message_count * 0.00001  # ~$0.01 per 1000 messages
    ...
    embed_label = "local, free" if embedding_provider == "local" else "OpenAI"
    print(f"\n  Estimated costs:")
    print(f"    Embeddings ({embed_label}):{' ' * max(1, 18 - len(embed_label))}${embed_cost:.2f}")
...
def main():
    ...
    if args.dry_run:
        ...
        estimate_costs(len(messages), embedding_provider=config.get("retrieval", {}).get("embedding_provider", "openai"))
        sys.exit(0)
    ...
    estimate_costs(msg_count, embedding_provider=config.get("retrieval", {}).get("embedding_provider", "openai"))

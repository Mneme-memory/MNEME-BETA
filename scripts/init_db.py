...
# Reconfigure stdout/stderr to UTF-8 so the checkmark/warning glyphs below
# don't raise UnicodeEncodeError on Windows' default cp1252 console. This
# must run before any print() call in this module, including the
# config-error handler in init_database() (which itself prints a glyph).
try:
    sys.stdout.reconfigure(encoding="utf-8")
    sys.stderr.reconfigure(encoding="utf-8")
except AttributeError:
    pass  # older Python or non-reconfigurable stream; glyphs may still fail but we don't crash on the guard itself

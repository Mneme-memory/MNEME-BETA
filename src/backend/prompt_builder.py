...
def resolve_instructions_path(config: Dict) -> Tuple[Path, str]:
    """Resolve the effective system-instructions file the runtime reads.

    Returns (path, source) where source is "profile" or "global".
    Mirrors the chain used by the AI at prompt-build time so the editor
    and runtime can never drift. NOTE: does NOT include the .example
    template — that is a template only, not a runtime source.
    """
    ...
    global_instructions = get_mneme_home() / "system_instructions.txt"
    return global_instructions, "global"
...
class PromptBuilder:
    ...
    def __init__(self, config: Dict, database=None, cache_manager=None):
        ...
        self.instructions_path, _ = resolve_instructions_path(config)
    ...
    def get_system_instructions(self) -> str:
        ...
        Loads from the active profile or Mneme-home system_instructions.txt
        and replaces:
    ...
    def _get_fallback_instructions(self) -> str:
        ...
        This should not normally be reached — setup copies
        system_instructions.example.txt into the Mneme home.
        """
...
IMPORTANT: This reminder is for your awareness only. Respond normally to their message, and create concepts using @concept create if appropriate. The concepts will persist across conversations to help you recognize similar patterns."""
...
IMPORTANT: This reminder is for your awareness only. Respond normally to their message first. Notes maintenance is secondary."""

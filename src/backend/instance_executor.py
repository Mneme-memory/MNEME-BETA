...
class InstanceExecutor:
    ...
    def execute_loop_stream(
        ...
        # Check for max iterations
        if iteration >= self.max_iterations:
            yield {"type": "notification", "data": f"  Max instance command iterations ({self.max_iterations}) reached"}
            return ai_response, commands_used
        ...
        yield {"type": "notification", "data": "🤖 Instance is processing commands..."}
        ...
        for command_str in found_commands:
            ...
            yield {"type": "notification", "data": f"   ↳ Instance used {cmd_display}"}
        ...
        if retrieval_result:
            ...
            yield {"type": "notification", "data": f"💭 Calling AI again with {result['command']} results..."}
            ...
            try:
                ...
                yield {"type": "notification", "data": f"⚠️  Streaming error, retrying without streaming..."}
    ...
    def _print_result(self, result: Dict):
        ...
        if result.get("success"):
            if result["command"] == "recall":
                ...
                data = result.get("data", {})
                print(f"      ↳ Artifact saved: {data.get('category', '?')}/{data.get('slug', '?')}")
    ...
    def _format_result_notification(self, result: Dict) -> str:
        ...
        if result.get("success"):
            if result["command"] == "recall":
                ...
                count = len(result.get("data", []))
                return f"      ↳ Retrieved {count} messages"
            elif result["command"] == "forget":
                data = result.get("data", {})
                if isinstance(data, dict) and data.get("previews"):
                    count = len(data["previews"])
                    return f"      ↳ Found {count} memories to review"
                return "      ↳ Archived to deep storage"
            elif result["command"] == "tag":
                return "      ↳ Tags updated"
            elif result["command"] == "modify":
                return f"      ↳ {result.get('message', 'Memory modified')}"
            elif result["command"] == "view":
                return "      ↳ Displaying message details"
            elif result["command"] == "concept":
                return f"      ↳ {result.get('message', 'Concept updated')}"
            elif result["command"] == "graph":
                return "      ↳ Graph query executed"
            elif result["command"] == "note":
                return f"      ↳ {result.get('message', 'Note updated')}"
            elif result["command"] == "config":
                return "      ↳ Configuration updated"
            elif result["command"] == "file":
                data = result.get("data", {})
                if data.get("is_retrieval"):
                    return f"      ↳ 📎 Loading file: {data.get('filename', 'unknown')}"
                else:
                    return f"      ↳ {result.get('message', 'File operation complete')}"
            elif result["command"] == "describe":
                data = result.get("data", {})
                if data.get("is_retrieval"):
                    return f"      ↳ 📎 Viewing file for description: {data.get('filename', 'unknown')}"
                else:
                    return f"      ↳ {result.get('message', 'Description updated')}"
            elif result["command"] == "artifact":
                data = result.get("data", {})
                return f"      ↳ Artifact saved: {data.get('category', '?')}/{data.get('slug', '?')}"
            elif result["command"] == "run":
                data = result.get("data", {})
                rc = data.get("return_code", "?")
                return f"      ↳ Script finished (exit code {rc})"
            else:
                return f"      ↳ {result.get('message', 'Executed successfully')}"
        else:

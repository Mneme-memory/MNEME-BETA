...
class CommandHandler:
    ...
    def __init__(self, database, embedder, config, background_queue, context_assembler=None, graph_db=None, file_processor=None, artifact_storage=None):
        ...
        Args:
            ...
            graph_db: GraphDatabase instance (for @graph commands, Phase 5)
    ...
    def cmd_recall(self, query: str) -> Dict:
        ...
        if deep:
            ...
            # Standard search (active + standard tiers only)
            results = retriever.semantic_search(
                query,
                limit=max_results,
                tier="standard",
    ...
    def _forget_confirm(self, args: str) -> Dict:
        ...
        if args.strip().lower() == "all":
            ids_to_archive = list(pending.keys()) if pending else []
        else:
            # Parse IDs from args
            ids_to_archive = []
            for token in args.split():
                try:
                    ids_to_archive.append(int(token))
                except ValueError:
                    continue
    ...
    # =========================================================================
    # @graph - Knowledge graph queries (Phase 5)
    # =========================================================================
    ...
    def cmd_graph(self, args: str) -> Dict:
        ...
        USAGE:
        @graph                 Show entity overview
        @graph consciousness   Show details for specific entity
        """
    ...
    def cmd_entity(self, args: str) -> Dict:
        """
        ...
        if sub_command == "list":
            ...
            if summary and summary.get("content"):
                ...
                response += "No summary generated yet."
            ...
            if not entity:
                return self._err("entity", f"No entity found matching '{name}'. Check @graph for entity names.")
...
TIPS & EDGE CASES:
  ...
  - @run auto-imports os; DB_PATH, ARTIFACTS_PATH, SRC_PATH available via os.environ
  - @run scripts are read-only and time out after 10 seconds
...
USAGE MODES:
1. @graph                    Show entity counts and statistics
2. @graph [entity name]      Show details for a specific entity
...
Examples:
  @graph                      Show entity overview
  @graph consciousness        Show details for "consciousness"
""",
            "note": """
@note <section_name>
<content>
...
"""
    ...
    def cmd_artifact(self, args: str) -> Dict:
        ...
        return self._ok("artifact", f"Artifact saved: {category}/{slug}/ARTIFACT.md", {
            "path": saved_path,
            "category": category,
            "slug": slug,
            "title": title,
    ...
    def cmd_run(self, args: str) -> Dict:
        """
        Execute a Python code snippet in a subprocess with read-only access
        to the database, artifacts directory, and source code.

        The code is written to a temp file and executed with environment
        variables providing paths. DB access is read-only (enforced via
        SQLite URI mode=ro). stdout/stderr are captured and returned.
        ...
        # Auto-prepend os import — nearly every script needs os.environ for DB_PATH etc.
        ...
        artifacts_dir = str(profile_dir / "artifacts")
        attachments_dir = str(profile_dir / "attachments")
        src_dir = str(Path(__file__).parent.parent.parent / "src")

        # Build read-only DB URI for sqlite3.connect()
        # file: URI with mode=ro enforces read-only at the SQLite level
        db_uri = Path(db_path).resolve().as_uri() + "?mode=ro"

        _os = __import__("os")
        # Start with minimal inherited env (platform-appropriate)
        inherited_keys = ["PATH", "HOME", "USER", "LANG", "TMPDIR",  # Unix
                          "SYSTEMROOT", "TEMP", "TMP", "USERPROFILE"]  # Windows
        env = {k: _os.environ[k] for k in inherited_keys if k in _os.environ}
        env.update({
            "DB_PATH": db_uri,
        ...
        try:
            ...
            # Retry once on failure (transient OneDrive file locks)
            ...
            return self._ok("run", f"Execution failed: {e}", {
                "is_retrieval": True,
                "output": "",
                "errors": str(e),
                "return_code": -1,
...
if __name__ == "__main__":
    ...
    print("  ✓ @graph - Query knowledge graph")

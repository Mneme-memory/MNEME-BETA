...
class DatabaseEntitiesMixin:
    ...
    def get_latest_monthly_summary(self, entity_id: int) -> Optional[Dict]:
        """Newest monthly summary for an entity (fallback when no all_time exists)."""
        results = self.execute_query("""
            SELECT * FROM entity_summaries
            WHERE entity_id = ? AND period_type = 'month'
    ...
    def merge_entity_links(self, target_id: int, source_ids: List[int]) -> int:
        """
        Repoint message_entities links from source entities to the target,
        deduplicating so no message ends up linked to the target twice.

        Part of entity merge: summaries for all involved entities should be
        deleted afterwards (delete_all_entity_summaries) so the lazy summary
        pipeline rebuilds the merged history from the combined links.

        Returns:
            Number of links now attached to the target.
        """
        if not source_ids:
            return 0
        placeholders = ",".join("?" for _ in source_ids)

        # Drop source links for messages the target already covers
        self.execute_write(f"""
            DELETE FROM message_entities
            WHERE entity_id IN ({placeholders})
              AND message_id IN (
                  SELECT message_id FROM message_entities WHERE entity_id = ?
              )
        """, (*source_ids, target_id))

        # Dedupe BEFORE repointing: a message linked to several sources must
        # keep only one source link, or the UPDATE below violates
        # UNIQUE(message_id, entity_id) mid-statement and aborts the merge.
        self.execute_write(f"""
            DELETE FROM message_entities
            WHERE entity_id IN ({placeholders})
              AND id NOT IN (
                  SELECT MIN(id) FROM message_entities
                  WHERE entity_id IN ({placeholders})
                  GROUP BY message_id
              )
        """, (*source_ids, *source_ids))

        # Repoint the rest — now guaranteed one link per message
        self.execute_write(f"""
            UPDATE message_entities SET entity_id = ?
            WHERE entity_id IN ({placeholders})
        """, (target_id, *source_ids))

        rows = self.execute_query(
            "SELECT COUNT(*) AS c FROM message_entities WHERE entity_id = ?",
            (target_id,)
        )
        return rows[0]["c"] if rows else 0

    def delete_all_entity_summaries(self, entity_ids: List[int]) -> None:
        """Delete every summary (monthly + all-time) for the given entities."""
        if not entity_ids:
            return
        placeholders = ",".join("?" for _ in entity_ids)
        self.execute_write(
            f"DELETE FROM entity_summaries WHERE entity_id IN ({placeholders})",
            tuple(entity_ids)
        )

...
class Database(
    ...
    def _auto_initialize_schema(self):
        ...
        # Ensure the graph/entity schema (entities table etc.) exists
        # regardless of entry point. message_entities, entity_summaries, and
        # attachments all declare FK REFERENCES entities(id), and
        # _get_connection() always sets `PRAGMA foreign_keys = ON`. If the
        # entities table is missing, SQLite refuses to even PREPARE any
        # "DELETE FROM messages ..." statement (it must resolve every FK
        # target that references the row, even if no matching child rows
        # exist), which silently breaks delete_message(),
        # cleanup_orphan_messages(), and TierManager's temporary-message
        # cleanup on any install that never explicitly constructs a
        # GraphDatabase (scripts/chat.py, importers, diagnostics, etc.).
        # graph_schema.sql is entirely `CREATE ... IF NOT EXISTS`, so this is
        # a cheap no-op both for fresh databases and for existing databases
        # that already have the graph schema applied.
        self._ensure_graph_schema()

    def _ensure_graph_schema(self):
        """
        Create the entity/graph tables (entities, entities_fts, indexes) if
        they don't already exist, using GraphDatabase's own schema file as
        the single source of truth (avoids duplicating CREATE TABLE
        statements here). Safe to call on every Database construction --
        graph_schema.sql only uses IF NOT EXISTS statements.
        """
        # Import here (not at module top) to avoid a circular import: some
        # graph_database.py-adjacent modules may import from database.py in
        # the future, and this keeps the dependency direction one-way at
        # construction time only.
        from .graph_database import GraphDatabase

        graph_db = GraphDatabase({"storage": {"database_path": self.db_path}})
        try:
    ...
    def get_message_page(
        ...
        """
        Keyset-paginated recent messages, newest first.

        Args:
            limit (int): Max messages to return.
            before_id (int, optional): If set, only return messages with
                id < before_id (i.e. the page immediately older than a
                previously-loaded message). If None, returns the most recent
                page (equivalent to the legacy get_recent_messages(limit)).

        Returns:
            (messages, has_more): messages is newest-first, length <= limit.
            has_more is True if older messages exist beyond this page.

        USE CASE:
        Backs incremental "load older messages" scrolling in the web UI
        (GET /api/history). Uses `id` (monotonically increasing on insert)
        rather than OFFSET so pagination stays O(limit) even on very long
        histories (8,000+ messages) instead of degrading with an O(n) scan.
        """
        params: List = []
        where_clause = ""
        ...
        # Fetch one extra row to detect whether more (older) messages remain
    ...
    def update_message_content(self, message_id: int, content: str) -> bool:
        ...
        """
        last_modified = datetime.now(timezone.utc).replace(microsecond=0).strftime('%Y-%m-%dT%H:%M:%S') + 'Z'
        query = "UPDATE messages SET content = ?, modified = 1, last_modified = ? WHERE id = ?"
        self.execute_write(query, (content, last_modified, message_id))
        return True
    ...
    def cleanup_orphan_messages(self, protect_trailing_turn: bool = False) -> dict:
        """
        ...
        Args:
            protect_trailing_turn: When True, the trailing turn is treated as an
                in-flight generation (durable stream reconnect v1) and is exempt
                from all deletion rules. This covers the just-stored user row and
                the pre-allocated empty assistant shell that ``conversation.py``
                creates before the API call. Without this guard, a page-load
                cleanup fired while a turn is still generating would delete the
                user row (and the empty shell), and the running turn's later
                ``update_message_content(ai_msg_id, ...)`` would silently vanish.
        ...
        # In-flight guard: compute the set of message ids belonging to the
        # trailing (currently generating) turn so no rule can touch them. The
        # in-flight turn is always the last row (user) optionally followed by a
        # pre-allocated empty assistant shell (its immediate successor).
        protected_ids = set()
        if protect_trailing_turn:
            idx = len(messages) - 1
            if messages[idx]["sender"] == "assistant" and messages[idx]["content"] == "":
                protected_ids.add(messages[idx]["id"])
                idx -= 1
            if idx >= 0 and messages[idx]["sender"] == "user":
                protected_ids.add(messages[idx]["id"])
        ...
        while i < len(messages):
            if messages[i]["sender"] == "user":
                ...
                if len(run) > 1:
                    for msg in run[:-1]:
                        if msg["id"] not in protected_ids:
                            ids_to_delete.append(msg["id"])
            else:
                i += 1
        ...
        if (surviving and surviving[-1]["sender"] == "user"
                and surviving[-1]["id"] not in protected_ids):

...
class ConversationManager:
    ...
    def _process_conversation_stream(self, user_input: str, sender: str, attachment_uuids: list = None):
        ...
        if processing_info.get("embedding_generated"):
            yield self._status("embedding", "embedded", "Searching your memories...")
        ...
        if entity_details and entity_details.get("success"):
            if entity_details.get("new_entity_names"):
                ...
                yield self._status(
                    "new_entities", "new entities", f"Noticed new topics: {names}",
                    detail={
            ...
            if links > 0:
                yield self._status("entity_links", "entities linked", f"Updated memory for {msgs} {'message' if msgs == 1 else 'messages'}")

        # Step 3: Assemble context
        yield self._status("context_assembly", "assembling", "Pulling together what I remember...")
        ...
        if memories_count > 0:
            ...
            if descriptions:
                ...
                summary = f"Found: {desc_str}"
            else:
                summary = f"Found {memories_count} relevant {'memory' if memories_count == 1 else 'memories'}"
            ...
            yield self._status("no_memories", "no matches", "No relevant memories found")

        if tc.get("entity_summaries", 0) > 0:
            bootstrapped = md.get("entities_bootstrapped", [])
            for name in bootstrapped:
                yield self._status("entity_bootstrapped", "entity created", f"Getting to know {name}...")
            ...
            if entity_names:
                ...
                summary = f"Remembered: {names_str}"
            else:
        ...
        if tc.get("notes", 0) > 0:
            ...
            yield self._status(
                "notes_loaded", "notes loaded", "Checked my notes",
                detail={
        ...
        if tc.get("concepts", 0) > 0:
            ...
            concept_summary = f"Recognized: {', '.join(n.replace('_', ' ') for n in concept_names)}" if concept_names else "Recognized patterns"
        ...
        if tc.get("files", 0) > 0:
            ...
            yield self._status(
                "files_loaded", "files loaded", f"Loaded {_file_count} {'attachment' if _file_count == 1 else 'attachments'}",
                detail={
        ...
        if tc.get("timeline", 0) > 0:
            ...
            yield self._status(
                "timeline_loaded", "timeline loaded", "Checked your recent week",
                detail={
        ...
        yield self._status(
            "token_budget", "context ready", "Memory assembled",
            detail={
        ...
        if attachment_blocks:
            ...
            yield self._status("attachments", "attachments", f"Loaded {_att_count} {'attachment' if _att_count == 1 else 'attachments'}")
        ...
        yield self._status(
            "calling_api", "calling API", "Sending...",
        ...
        try:
            ...
            if usage_data:
                ...
                if cache_read > 0:
                    ...
                    pill = "cache written"
                    summary = "Cache updated — next turn will be faster"
                else:
                    pill = "no cache"
        ...
        yield self._status("finalizing", "finalizing", "Saving to memory...")
        ...
        if ai_processing_info.get("entity_batch_processed"):
            ...
            yield self._status("entity_batch", "memory updated", f"Memory updated ({bg_stats['entity_links_created']} connections)")

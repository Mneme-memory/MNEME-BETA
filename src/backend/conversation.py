...
class ConversationManager:
    ...
    def _process_conversation_stream(self, user_input: str, sender: str, attachment_uuids: list = None):
        ...
        if self.last_total_prompt_tokens >= 195_000:
            yield self._status("cache_overflow", "trimming", "Running low on space — archiving older messages")
            self.tier_manager.transition_tiers(background_summaries=True)
            self.conversation_history = self._load_recent_context()
            yield self._status("context_trimmed", "trimmed", f"Trimmed to {len(self.conversation_history)} recent messages")
        elif self.last_total_prompt_tokens >= 185_000:
            yield self._status("cache_warning", "context full", "Getting close to memory limit — a fresh session soon would help")
    ...
    def _call_ai(self, prompt: str = None, context: Dict = None, current_input: str = None, stream: bool = False, attachment_blocks: list = None):
        ...
        try:
            ...
            if use_caching and context is not None and current_input is not None:
                ...
                # Cold start - use structured messages without cache_control.
                # This keeps image/file blocks in the same Anthropic-compatible
                # shape as cached calls while avoiding prompt-cache writes.
                if prompt is None:
                    if context is not None and current_input is not None:
                        ...
                        history_attachments = self._load_history_attachment_blocks(self.conversation_history)
                        no_cache_structure = self.prompt_builder.build_cached_structure(
                            sections=sections,
                            conversation_history=self.conversation_history,
                            current_input=current_input,
                            concept_reminder=combined_reminder or None,
                        ...
                        system_param = self._strip_cache_control(no_cache_structure["system"])
                        messages = self._strip_cache_control(no_cache_structure["messages"])

...
This module generates vector embeddings for messages. New Mneme installs use
the bundled local BGE model; a legacy OpenAI provider remains available for
existing configurations while they migrate.
...
PROVIDERS:
- Local (default for new installs): BAAI/bge-small-en-v1.5 via ONNX, 384
  dimensions, no API key or per-message cost
- OpenAI (legacy): text-embedding-3-small, 1536 dimensions, requires an
  OpenAI API key
...
USAGE:
    ...
    gen = create_embedding_generator(load_config())
    embedding = gen.generate_embedding("Tell me about the crows")
    # Returns a vector whose dimensions depend on the configured provider
"""
...
def create_embedding_generator(config: Dict):
    ...
    point (server.py, scripts/import_conversation.py, and diagnostics) should
...
if __name__ == "__main__":
    ...
    print("\nTo use this module:")
    print("1. Keep retrieval.embedding_provider set to 'local' (the new-install default)")
    print("2. Import and use:")
    print("   from embeddings import create_embedding_generator")
    print("   from config import load_config")
    print("   gen = create_embedding_generator(load_config())")

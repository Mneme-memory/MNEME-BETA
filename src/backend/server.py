...
from src.backend.config import load_config, save_config, save_profile_config, update_active_profile, get_backup_path
...
def set_startup_status(state=None, phase=None, detail=None, error=None, background_work=None):
...
def check_setup_mode():
    ...
    if request.path in ('/api/setup', '/api/setup/status', '/api/startup/status'):
        return None
...
def update_settings():
    ...
    try:
        ...
        if 'recent_messages_tokens' in data:
            ...
            if not 10000 <= val <= 150000:
                return jsonify({"error": "recent_messages_tokens must be between 10000 and 150000"}), 400
